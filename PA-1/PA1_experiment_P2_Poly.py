...
def main():
    ...
    nopara_dict={'function':'poly','order':1,'Lambda':0}
    lambda_candict = {'Lambda':[0.1,0.25,0.5,1,2,5],'function':['poly'],'order':[2]}
    BR_candict = {'alpha':[0.1,0.5,1,5],'sigma':[0.1,0.5,1,5],'function':['poly'],'order':[2]}
    ...
    for key, value in NAME_MAP.items():
        ...
        if (key=='RLS' or key =='LASSO'):
            ...
            imp.mseMap_toCSV(para_err,'P2_mse_poly'+key+'.csv')
        ...
        mse,prediction = imp.experiment(testx,testy,trainx,trainy,paradict=params,method=key,plot_title='P2_poly1'+value,show_plot=False)
